...
<div class="row">
	...
	<div class="col-xl-6 col-12">
		<div class="text-center">
			<ul class="list-group mb-3 text-sm text-nowrap wg-account-action-list">
				...
				{{{ if isSelf }}}
				<li class="list-group-item"><a href="{config.westgateAccount.manageUrl}" class="text-decoration-none text-reset" target="_top" rel="nofollow">Manage Westgate account</a></li>
				<li class="list-group-item"><a href="{config.relative_path}/user/{userslug}/edit/password" class="text-decoration-none text-reset">Change password</a></li>
				<li class="list-group-item"><a href="{config.relative_path}/user/{userslug}/settings" class="text-decoration-none text-reset">Two-factor authentication</a></li>
				<li class="list-group-item"><a href="{config.relative_path}/user/{userslug}/sessions" class="text-decoration-none text-reset">Forum sessions</a></li>
				{{{ else }}}
			...
			{{{ if isSelf }}}
			<p class="wg-account-note text-start">
				Username, email, password, and multi-factor settings are part of the shared Westgate account. Forum-only profile details can still be edited here.
			</p>
			{{{ end }}}
		...
		{{{ if sso.length }}}
		<label class="form-label text-sm fw-semibold">Westgate sign-in links</label>
		<div class="list-group">
			...
			<div class="list-group-item d-flex align-items-center justify-content-between">
				<a class="text-sm text-reset text-decoration-none" data-component="{./component}" href="{{{ if ./url }}}{./url}{{{ else }}}#{{{ end }}}" target="{{{ if ./associated }}}_blank{{{ else }}}_top{{{ end }}}">
					...
					{{{ if ./associated }}}Linked{{{ else }}}Not linked{{{ end }}}
					{./name}
				</a>
				{{{ if ./deauthUrl }}}
